...
class TestTicketLifecycle:
    ...
    async def test_ticket_resale(self, api_client, w3, ticket_contract):
        ...
        assert resale_result["owner_address"].lower() == resale_data["to_address"].lower()
    ...
    async def test_ticket_invalidation(self, api_client, w3, ticket_contract):
        ...
        response = await api_client.post("/api/v1/tickets/invalidated", json=invalidate_data)
        assert response.status_code == 200
    ...
    async def test_error_handling(self, api_client):
        ...
        response = await api_client.post(
            "/api/v1/tickets/checked-in",
            json={"ticket_id": "non-existent-ticket"}
        )
