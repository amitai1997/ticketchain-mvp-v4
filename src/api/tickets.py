...
# Import the ticket registry
from ..datastore.ticket_registry import ticket_registry as registry


@router.post("/sold", response_model=TicketResponse, status_code=status.HTTP_201_CREATED)
async def sold_ticket(
    request: SoldTicketRequest,
    blockchain_service: Annotated[BlockchainServiceInterface, Depends(get_blockchain_service)],
) -> TicketResponse:
    """
    ...
    try:
        ...
        metadata = {
            ...
            "description": request.description or f"Ticket for event {request.event_id}",
            "image": request.image_url,
            "attributes": request.metadata_attributes or {},
            "event_id": request.event_id,
            "ticket_id": request.ticket_id,
        }
        token_uri = f"data:application/json;base64,{json.dumps(metadata).encode().hex()}"
